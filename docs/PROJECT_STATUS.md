...
5. **URL-monitoring watcher** (the actual "watching for a change" trigger for the Vercel pipeline — right now a document lands in `incoming-uploads` but nothing publishes `nocap-document-ingested` for it automatically, the smoke test above did that step by hand). Decided against a Google Drive-based watcher since it didn't remove the human step. Not started.
6. **Notification system** — in-app + email (Resend) — not started.
7. **Hard submission requirements that don't exist yet regardless of any code work**: demo video, architecture diagram, Devpost writeup. Not started.
