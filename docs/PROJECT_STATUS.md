...
5. ~~Notification system~~ **Done.** In-app bell (`NotificationBell.tsx`/`useNotifications.ts`) lives in `DashboardHeader`, backed by Supabase Realtime on the real `notifications` table; `notifier.py` (both `backend/` and the Vercel `frontend/api/_lib/` copy) inserts a row and sends a Resend email at the same points Inspector/Verifier already call it. Re-confirmed working 2026-08-31 after a teammate's merge briefly reverted this to a dead Firebase integration that was never wired up, see git history (`5ejYQuA`, `6P3m5wkUn5`, `D7DFX3da1`, `1842ae3`).
...
7. **The GitHub repo was transferred to the `NoCap-ATAH-26` org on 2026-09-01, and Vercel's auto-deploy on push broke as a result** — the project's Git connection still pointed at the old `bhumika-singhh/ATAH-26` location, so pushes stopped triggering builds. Reconnecting the repo in Vercel's Git settings and re-authorizing the Vercel GitHub App on the new org has been done, but redeploys were still failing with `errorCode: git_info_fail` as of this note. Confirm this is actually resolved before relying on auto-deploy again.
