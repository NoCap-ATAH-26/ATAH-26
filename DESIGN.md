...
The core palette is four hues plus true black, and each hue has exactly one job. This is a codified system, not a proposed one — the values below are what's already shipped, examined for consistency and audited for contrast rather than reinvented. A fifth and sixth hue, Blue and Gold, were added later as **decorative-only** additions for the dashboard's glow system (see Elevation & Depth) — they carry no interaction or status meaning and exist purely so the working dashboard reads as multicolor rather than a two-hue repeat of Primary and Secondary.
...
- **Accent-blue (#4FA3FF) and Accent-gold (#E8B94B)** — decorative-only, dashboard-glow-only. Unlike Primary and Secondary, these carry no interaction or brand-voice meaning and unlike Good/Warning/Serious/Error they carry no outcome meaning — their only job is variety, spreading a fourth and fifth hue across the pipeline stages and the ambient background wash so the surface reads as multicolor. Precisely because they don't mean anything, they must never appear on a button, a link, a focus ring, or a status badge — the moment one does, a viewer will reasonably read it as a fifth outcome or a new interactive affordance, and both readings are wrong.
...
3. **Colored glow**, on the panels the product wants to read as instruments that are actively doing something, not just holding content. This is a system-level device now, not a one-off, and it deliberately spans four hues rather than one so the dashboard reads as multicolor: `glow-mint` (Aquamarine) marks the Risk Score chart's card and line and the Ingested pipeline stage; `glow-mauve` (Dusty Mauve) marks the Activity Feed, matching mauve's existing job as the system's one warm, human-facing hue, and the Verifier pipeline stage; `glow-blue` and `glow-gold` — the two decorative-only hues — mark the Inspector and Repair pipeline stages respectively, and the Chat button on hover. Each of the four pipeline stages carries its own hue, left to right, rather than repeating one color across all four — the strip reads as a small spectrum. A softer version of all four hues also blooms as four large, fixed, blurred radial gradients behind the whole dashboard (`.ambient-glow`) — dim, out-of-focus color pools in the dark, not a lit UI element in their own right. Status badges and stat-tile icons carry a small glow in their *own* assigned color (good/warning/serious/error, or blue for the one neutral tile), which is a fifth pattern, not the four glow utilities — a glow there is read the same way the fill already is: it names the outcome (or, for the one non-status tile, just supplies a hue), it doesn't decorate it.
...
**Glow.** Four fixed utilities, `glow-mint`, `glow-mauve`, `glow-blue`, `glow-gold` — each a solid-color 1px border plus three stacked outward shadows (tight, medium, wide) in the same hue, never a neutral shadow with color layered on top. Applied at the card level to a fixed, short list of panels: Risk Score chart → mint; Activity Feed → mauve; the four pipeline stage nodes each get their own of the four hues, left to right. Never used as a generic "make this card pop" treatment — a panel with no assigned role stays on tonal layering and a hairline border. The chart's line gets a matching glow via an SVG `feGaussianBlur`/`feMerge` filter rather than a CSS `box-shadow` or `filter: drop-shadow`, since Recharts' stroke isn't a stable DOM node for CSS filters to target — the SVG filter is the correct primitive for glowing a vector path, the CSS utilities are the correct primitive for glowing a card. Status badges and stat-tile icons use a fifth pattern, an inline `box-shadow` computed from that element's own status/assigned color rather than the four fixed utilities — the glow there is naming an outcome (or, for the one neutral stat tile, just supplying a hue), not a brand moment.
...
- **Do** reserve colored glow (`glow-mint`, `glow-mauve`, `glow-blue`, `glow-gold`, and status-colored badge/icon halos) for the fixed, short list of surfaces it's assigned to — live data, the narrative panel, each pipeline stage, and outcome colors. It's a meaning-carrying device like `primary`, not a default "nicer card" upgrade; a card with no assigned role stays flat.
- **Don't** put Accent-blue or Accent-gold on a button, link, focus ring, or status badge. They're decorative-only, reserved for glow — putting either on an interactive or status element makes it look like a fifth pipeline outcome or a new kind of button, and it's neither.
