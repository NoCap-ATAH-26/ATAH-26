...
satisfied by deploying the pipeline itself on Cloud Run, not by this table,
Supabase is not a Google Cloud service.
...
and the pipeline keeps running without it, so teammates without a .env set
up yet aren't blocked from testing the agents.
"""
...
def _get_client():
    ...
    if not url or not key:
        if not _warned:
            print(
                "[audit_log] SUPABASE_URL/SUPABASE_KEY not set in .env, "
                "audit logging disabled. See backend/audit_log.py docstring.",
                file=sys.stderr,
            )
            _warned = True
        return None
