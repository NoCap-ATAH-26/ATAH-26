...
interface SmokeyCursorProps {
  ...
  followMouse?: boolean;
  autoColors?: boolean;
  // Fixed hex color (e.g. "#E7DADA") for every smoke splat. Omit for the
  // library's default behavior: a fresh random hue per splat/cycle.
  color?: string;
}
...
function hexToRGB01(hex: string): ColorRGB {
...
export default function SmokeyCursor({
  ...
  useEffect(() => {
    ...
    function initFramebuffers() {
      ...
      // Non-null: RGBA/RG/R always resolve to a supported format on any
      // real WebGL implementation (getSupportedFormat only returns null if
      // every fallback in the chain is rejected, which doesn't happen in
      // practice — same guarantee the rest of this file leans on via `!`).
    ...
    // Fixed hex color when `color` is set, otherwise a fresh random hue —
    // this is the only real behavioral fork from the upstream component,
    // which declares `autoColors`/`intensity` but never wires them up.
    function generateColor(): ColorRGB {
      if (color) {
        const fixed = hexToRGB01(color);
        return {
          r: fixed.r * 0.15 * intensity,
          g: fixed.g * 0.15 * intensity,
          b: fixed.b * 0.15 * intensity,
        };
      }
